...
jqueryWidget: {
    _init: function() {
        ...
        if (typeof(this.options.s) == "string")
            this.words = this.options.s.split("|");
        else {
...
//                    t.recordSprResult(time, t.currentWord);
		    ...
		    regiontime =  new Date().getTime() - oldregiontime
		    oldregiontime = new Date().getTime()		   
		    region_RT.push(regioncounter)
		    region_RT.push(regiontime)
	...
	for (var i = 0; i < this.sprResults.length; ++i) {
	    ...
	    this.resultsLines.push([              
	        ["Reading time", region_RT.slice(2,12).toString()],
		["Word number", i+1],
